...
const Tracker = ({ sectionActive }: { sectionActive: string }) => {
  const stack = useRef<HTMLAnchorElement | null>(null);
  const frameworks = useRef<HTMLAnchorElement | null>(null);
  const additional = useRef<HTMLAnchorElement | null>(null);
  ...
  useEffect(() => {
    elementIds.forEach((dataTarget) => {
      ...
      if (sectionActive === "skills" || window.scrollY > 2300) {
        if (stack.current) stack.current.style.background = "#e2e8f066";
        ...
        if (stack.current) stack.current.style.background = "";
        if (frameworks.current) frameworks.current.style.background = "";
        if (additional.current) additional.current.style.background = "";
      }
